...
internal class SSConfigurationManagerAttributes
{
...
public class ConfigSync
{
	...
	public SyncedConfigEntry<T> AddConfigEntry<T>(ConfigEntry<T> configEntry)
	{
		if (configData(configEntry) is not SyncedConfigEntry<T> syncedEntry)
		{
			...
			AccessTools.DeclaredField(typeof(ConfigDescription), "<Tags>k__BackingField").SetValue(configEntry.Description, new object[] { new SSConfigurationManagerAttributes() }.Concat(configEntry.Description.Tags ?? Array.Empty<object>()).Concat(new[] { syncedEntry }).ToArray());
	...
	private void serverLockedSettingChanged()
	{
		...
		{
			configAttribute<SSConfigurationManagerAttributes>(configEntryBase.BaseConfig).ReadOnly = !isWritableConfig(configEntryBase);
		}
	}
