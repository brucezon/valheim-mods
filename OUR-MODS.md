...
19. Stars — `Star chance by progress` (On), `Per boss over biome` (0.2), `Max multiplier` (2), `Boss keys`.
    ...
    (m_maxLevel) untouched. CLLC-Fluid idea on vanilla's own roll; CLLC itself was tried and removed 16 Sep
    (its multiplayer scaling multiplies the game's result assuming stale 0.4/0.04 constants; its range
    transpiler is dead on 1.0).
