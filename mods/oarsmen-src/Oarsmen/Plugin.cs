...
public class OarsmenPlugin : BaseUnityPlugin
{
	...
	private void Awake()
	{
		...
		RowUnderSail = config("2 - Rowing", "Row under sail", Toggle.Off, "On = rowers also add their forward force while the sail is half or fully out, falling away with speed as set below. Off = rowing only counts in paddle mode and reverse. Steering help is never added under sail either way.");
		RowCutoutSpeed = config("2 - Rowing", "Rowing cuts out above (m/s)", 5f, "Only used when 'Row under sail' is on. An oar only bites while the blade is moving through the water faster than the hull is, so the crew's help falls away as the ship picks up speed and is worth nothing at all at this one: thrust scales as (1 - speed/this) squared. At 5 that is a full share becalmed, about a third at 2 m/s and nothing by 4 - rowing gets you out of a calm or a foul wind and is futile once the sail is really pulling. Set it near two thirds of your hull's full-sail speed, which 'oarsmen ship' will tell you. Paddle mode and reverse are not affected; they keep vanilla's own flat paddle force. 0 = no falloff, a flat share at any speed.");
